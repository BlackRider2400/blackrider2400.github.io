$(document).ready(function () {
  const apiRoot = "https://mylovelyserver.fun:8443/crud/v1/";
  const trelloApiRoot = "https://mylovelyserver.fun:8443/crud/v1/trello/";
  ...
  function createElement(data) {
    ...
    element
      .find("[data-task-name-section] [data-task-name-paragraph]")
      .text(data.title);
    element
      .find("[data-task-name-section] [data-task-name-input]")
      .val(data.title);

    element
      .find("[data-task-content-section] [data-task-content-paragraph]")
      .text(data.content);
    element
      .find("[data-task-content-section] [data-task-content-input]")
      .val(data.content);
  ...
  function getAllTasks() {
    const requestUrl = apiRoot + "task/getTasks";
  ...
  function handleTaskUpdateRequest() {
    ...
    var requestUrl = apiRoot + "task" + "/updateTask";
    ...
    $.ajax({
      ...
      success: function (data) {
        parentEl
          .attr("data-task-id", data.id)
          .toggleClass("datatable__row--editing");
  ...
  function handleTaskDeleteRequest() {
    var parentEl = $(this).parents("[data-task-id]");
    var taskId = parentEl.attr("data-task-id");
    var requestUrl = apiRoot + "task" + "/deleteTask";
  ...
  function handleTaskSubmitRequest(event) {
    ...
    var requestUrl = apiRoot + "task" + "/createTask";
  ...
  function handleBoardNameSelect(event) {
    ...
    var $listNameSelectEl = $changedSelectEl.siblings(
      "[data-list-name-select]",
    );
    var preparedListOptions = prepareBoardOrListSelectOptions(
      availableBoards[selectedBoardId].lists,
    );
  ...
  $tasksContainer.on(
    "change",
    "[data-board-name-select]",
    handleBoardNameSelect,
  );
  $tasksContainer.on(
    "click",
    "[data-trello-card-creation-trigger]",
    handleCardCreationRequest,
  );
  $tasksContainer.on("click", "[data-task-edit-button]", toggleEditingState);
  $tasksContainer.on(
    "click",
    "[data-task-edit-abort-button]",
    toggleEditingState,
  );
  $tasksContainer.on(
    "click",
    "[data-task-submit-update-button]",
